from myradarforecast.utils import UnicodeMixin, PropertyUnavailable
import datetime
import requests
...
class Forecast(UnicodeMixin):
    ...
    def _myradarforecast_data(self, key):
        ...
        try:
            ...
            if key == 'currently':
                return MyradarforecastDataPoint(self.json[key])
            else:
                return MyradarforecastDataBlock(self.json[key])
        except:
            if key == 'currently':
                return MyradarforecastDataPoint()
            else:
                return MyradarforecastDataBlock()


class MyradarforecastDataBlock(UnicodeMixin):
    ...
    def __init__(self, d=None):
        ...
        self.data = [MyradarforecastDataPoint(datapoint)
                     for datapoint in d.get('data', [])]

    def __unicode__(self):
        return '<MyradarforecastDataBlock instance: ' \
               '%s with %d MyradarforecastDataPoints>' % (self.summary,
                                                     len(self.data),)


class MyradarforecastDataPoint(UnicodeMixin):
    ...
    def __unicode__(self):
        return '<MyradarforecastDataPoint instance: ' \
